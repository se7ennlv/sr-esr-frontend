...
@Component({
  selector: 'app-srn-rpt',
  templateUrl: './srn-rpt.component.html',
  styleUrls: ['./style.scss']
})
export class SrnRptComponent implements OnInit {
  includeForm: boolean;
  ...
  onUpdate(item: ISRN) {
    ...
    Object.assign(this.surveyService.srnModel, item);
  ...
  onDelete(id: any) {
    this.confirmationService.confirm({
      ...
      accept: () => {
        this.surveyService.delete(id, 'srn').subscribe((res) => {
  ...
  fetchData() {
    ...
    this.surveyService.get('srn', fromDate, toDate).subscribe((res) => {
      if (res.status === 'success') {
        this.items = res.data;
      }
    });
  ...
  exportExcel() {
    import('xlsx').then(xlsx => {
      ...
      this.saveAsExcelFile(excelBuffer, 'sr-tracking-new');
    });
  }
